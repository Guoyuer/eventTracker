...
**Task 1–5 已完成**（最近为 sealed Record 重构），**从 Task 6 继续**。Task 6 已完成生成器、应用 shell 与失败消息的本地化基础，但全 UI 迁移和防回归守卫仍未完成。具体执行顺序以 `docs/plans/2026-07-10-unified-quality-execution.md` 为准；本文件保留每个生产级缺口的设计与验收细节。每个已完成 Task 都经过独立 spec+quality 审查。执行中的发现，后续 Task 必须遵守：

- **当前基线已变**：`flutter test` 是 **122 个测试**。analyze 依旧必须 `No issues found!`（CI 用 `--fatal-infos`）。
...
- 每个 Task 结束时 `flutter analyze` 必须 **`No issues found!`**，`flutter test` 必须全绿。当前基线：analyze 干净，**122 个测试全部通过**（21 个测试文件）。
